...
# Entorno dev.
#
# Perfil: barato y desechable. Sin proteccion contra borrado, sin PITR, sin
# instancias minimas, Object Lock en modo GOVERNANCE (eludible con permiso) y
# retencion de logs corta.
#
...
module "aws_kms" {
  ...
  env                        = var.env
  tenants                    = var.tenants
  deletion_window_in_days    = 7 # el minimo admitido: en dev se destruye rapido
...
module "aws_networking" {
  ...
  # Los endpoints de tipo Interface se facturan por hora y por zona: en dev no
  # compensan.
...
module "aws_storage" {
  ...
  # GOVERNANCE permite eludir la retencion con permiso explicito. En dev es
  # obligatorio: con COMPLIANCE el bucket seria indestructible.
...
module "aws_compute" {
  ...
  env                 = var.env
  inference_memory_mb = 2048 # en dev basta para pruebas funcionales
...
module "aws_orchestration" {
  ...
  human_review_timeout_seconds = 86400 # 1 dia: en dev nadie revisa el fin de semana
  log_retention_days           = 7
  log_level                    = "ALL"
  include_execution_data       = true # aceptable solo porque en dev no hay PII real
}
...
module "aws_api" {
  ...
  authorizer_cache_ttl_seconds = 0 # sin cache: facilita depurar cambios de politica
  enable_waf                   = false
  log_retention_days           = 7
}
...
module "aws_gdpr" {
  ...
  enable_retention_sweep = false # en dev no hay retencion que barrer
}
...
module "gcp_kms" {
  ...
  # Minimo practico en dev para poder reciclar el proyecto.
...
module "gcp_networking" {
  ...
  enable_psc_google_apis   = false # en dev no compensa
...
module "gcp_data" {
  ...
  # Nombre propio en lugar de (default): la base de datos por defecto no puede
  # eliminarse, y en dev hay que poder destruir el entorno entero.
...
module "gcp_storage" {
  ...
  evidence_retention_years = 1
  lock_evidence_retention  = false # irreversible: nunca en dev
  allow_force_destroy      = true
...
module "gcp_orchestration" {
  ...
  call_log_level       = "LOG_ALL_CALLS" # aceptable solo porque en dev no hay PII real
