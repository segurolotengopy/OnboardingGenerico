...
# Entorno prd.
#
# Perfil: todo lo que en dev se apaga por coste, aqui se enciende por
# obligacion.
#
#   - prevent_destroy en los datos y en las llaves (protect_from_destroy = true
#     en el modulo de datos; las llaves de Cloud KMS lo llevan siempre porque el
#     servicio no permite borrarlas).
#   - PITR y proteccion de borrado en tablas y bases de datos.
#   - Object Lock en modo COMPLIANCE: la retencion no puede eludirla nadie, ni
#     el root de la cuenta. Es irreversible por objeto.
#   - Bucket Lock bloqueado en Cloud Storage: tambien irreversible.
#   - Retencion de logs de 400 dias, alineada con el bucket _Required de Cloud
#     Logging y por encima de los 90 dias de historial de Step Functions.
#   - Instancias minimas mayores que cero para eliminar el arranque en frio.
#   - WAF activo y sin volcado de datos de ejecucion a los logs.
#
# ANTES DE APLICAR POR PRIMERA VEZ, lea las advertencias de irreversibilidad en
# los README de los modulos aws/storage, gcp/storage y aws/kms.
#
...
module "aws_kms" {
  ...
  env     = var.env
  tenants = var.tenants
  # Ventana maxima: el crypto-shredding de un tenant debe poder revertirse si la
  # solicitud resulta ser fraudulenta o erronea.
...
module "aws_networking" {
  ...
  # Solo si algun adaptador llama a un proveedor SaaS externo. Manteniendolo en
  # false, el egreso a Internet es imposible por construccion.
  enable_nat_gateway = var.enable_internet_egress
}
...
module "aws_data" {
  ...
  # Activa la variante de la tabla con lifecycle prevent_destroy. NO cambie este
  # valor sobre una tabla con datos: fuerza destruccion y recreacion.
...
module "aws_storage" {
  ...
  biometric_retention_days = var.biometric_retention_days
  staging_retention_days   = 7

  # COMPLIANCE: nadie puede eludir la retencion, ni el root de la cuenta. Es
  # IRREVERSIBLE por objeto. La unica forma de dejar esta evidencia ilegible es
  # el crypto-shredding de la CMK del tenant.
  evidence_object_lock_mode = "COMPLIANCE"
  evidence_retention_years  = var.evidence_retention_years
...
module "aws_identity" {
  ...
  platform_role_trusted_principals = var.platform_role_trusted_principals
}
...
module "aws_compute" {
  ...
  env = var.env
  # Valor de partida sujeto al perfilado descrito en el README de aws/compute.
  # El rango real de Lambda es 128 a 10240 MB y la vCPU es proporcional a la
  # memoria: el optimo se mide, no se hereda.
  inference_memory_mb = var.inference_memory_mb
...
module "aws_orchestration" {
  ...
  # Debe cubrir fin de semana, festivo y escalado a compliance. El tiempo de
  # espera no se factura: solo se paga la transicion.
  human_review_timeout_seconds = 604800 # 7 dias
  log_retention_days           = 400
  log_level                    = "ERROR"
  # NUNCA true en produccion: volcaria punteros y metadatos de PII a CloudWatch.
...
module "aws_api" {
  ...
  # Cache corta: ahorra invocaciones sin retrasar demasiado la revocacion de un
  # token comprometido.
  authorizer_cache_ttl_seconds = 60
  enable_waf                   = true
  log_retention_days           = 400
}
...
module "gcp_kms" {
  ...
  # 30 dias, el valor por defecto de Cloud KMS. PENDIENTE DE VERIFICAR el
  # minimo configurable antes de comprometer cualquier SLA de borrado.
...
module "gcp_storage" {
  ...
  evidence_retention_years = var.evidence_retention_years
  # IRREVERSIBLE: una politica bloqueada solo puede alargarse, y el bucket no
  # podra borrarse hasta que expire la retencion de todos sus objetos.
  lock_evidence_retention = true
  allow_force_destroy     = false
...
module "gcp_orchestration" {
  ...
  # LOG_ALL_CALLS registraria cuerpos de peticion con metadatos de PII.
  call_log_level       = "LOG_ERRORS_ONLY"
  event_retention_days = 31 # maximo del servicio
  labels               = local.common_labels
}
...
module "gcp_observability" {
  ...
  long_term_sink_bucket    = module.gcp_storage[0].evidence_bucket_name
